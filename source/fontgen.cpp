...
int CFontGen::SaveFont(const char *szFile)
{
	...
	if( _stricmp(filename.substr(filename.length() - 4).c_str(), ".dif") == 0 )
		filename = filename.substr(0, filename.length() - 4);

	errno_t e = fopen_s(&f, (filename + ".dif").c_str(), "wb");
	if( e != 0 || f == 0 )
		return -1;
...
#pragma pack(pop)
	...
	{
		string str = acStringFormat("%s.%s", filename.c_str(), textureFormat.c_str());
...
int CFontGen::LoadConfiguration(const char *filename)
{
	...
	int    _fontDescFormat;         config.GetAttrAsInt("fontDescFormat", _fontDescFormat, 0, 3);
	bool   _fourChnlPacked;         config.GetAttrAsBool("fourChnlPacked", _fourChnlPacked, 0, false);
	string _textureFormat;          config.GetAttrAsString("textureFormat", _textureFormat, 0, "dds");
